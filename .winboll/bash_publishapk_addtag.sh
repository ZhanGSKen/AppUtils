...
function addWorkflowsTag {
	...
	BASE_BETA_VERSION=$(grep -o "baseBetaVersion=.*" .winboll/build_flag.properties | awk -F '=' '{print $2}')
	echo "< .winboll/build_flag.properties baseBetaVersion : ${BASE_BETA_VERSION} >"
	## 设新的 workflows 标签
	# 脚本调试时使用
	#tag="v7.6.4-beta"
	# 正式设置标签时使用
